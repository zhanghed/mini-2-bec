...
Page({
  ...
  click_block: function (e) {
    let that = this
    // 输入框展示日期用
    let data = e.currentTarget.dataset.pid2
    let k = e.currentTarget.dataset.pid //2024z2z23
    wx.showModal({
      title: data.sYear + '年' + data.sMonth + '月' + data.sDay + "日",
      editable: true,
      placeholderText: '请输入内容(12个字以内)',
      success: function (res) {
        if (res.confirm) {
          let txt = res.content.slice(0, 12)
          if (Object.getOwnPropertyNames(App.sss).length >= 600) { //判断记录数
            wx.showModal({
              title: '记录数量过多，请及时清理',
            })
          } else {
