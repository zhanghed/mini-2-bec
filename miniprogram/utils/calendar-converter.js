...
var solarTermIdx =
  "0123415341536789:;<9:=<>:=1>?012@015@015@015AB78CDE8CD=1FD01GH01GH01IH01IJ0KLMN;LMBEOPDQRST0RUH0RVH0RWH0RWM0XYMNZ[MB\\]PT^_ST`_WH`_WH`_WM`_WM`aYMbc[Mde]Sfe]gfh_gih_Wih_WjhaWjka[jkl[jmn]ope]qph_qrh_sth_W";
var solarTermOS =
...
function addFstv(
  ...
  var dayCyclical =
    Date.UTC(sYear, sMonth, 1, 0, 0, 0, 0) / 86400000 + 25567 + 10;
  //日柱
  ...
  for (i = 0, item; (item = wFtv[i]); i++) {
    if (item.match(/^(\d{2})(\d)(\d)([\s\*])(.+)$/)) {
      if (Number(RegExp.$1) == sMonth + 1) {
        tmp1 = Number(RegExp.$2); //第几周
        tmp2 = Number(RegExp.$3); //周几
        if (tmp1 < 5) {
          var wFtvDate = (tmp2 == 0 ? 7 : 0) + (tmp1 - 1) * 7 + tmp2;

          //修改的地方 原来的没有计算当前月空出的天数
          var weekk = new Date(sYear, sMonth, 1); //获取当前月的1号是周几（这个月的第一天）。月份需要减1
          weekk = weekk.getDay(); //第一天在周几
          weekk = weekk == 0 ? 7 : weekk; // 修

          if (wFtvDate == sDay + weekk - 1) {
  ...
  for (i = 0, item; (item = lFtv[i]); i++) {
    if (item.match(/^(\d{2})(.{2})([\s\*])(.+)$/)) {
      ...
      if (
        (tmp1 == lunarMonth && tmp2 == lunarDay) ||
        (tmp2 == "00" && lunarMonth == 12 && lMonLen == lunarDay)
      ) {
